...
var setTranscriptomicCovbins = function(file, fasta, filesUploading,callback) {
...
var updateTranscriptomicCovbins = function(file, data, lastChunk, filesUploading) {
  ...
  if (lastChunk) {
    serializeTranscriptomicCovbins(file, filesUploading);
  }

}

var serializeTranscriptomicCovbins = function(file, filesUploading) {
...
var calculateTranscriptomicCovDensity = function(transcriptomicCov, fasta, genomeLength, start, end) {
//  console.log(calculateTranscriptomicCovDensity)
//  console.log(genomeLength,start,end)
  var regionSize = genomeLength / nbDiv;
  ...
  for (var j = 0; j < transcriptomicCov.length; j++) {
    for (var k = 0; k < chromosomesArray.length; k++) {

      if (transcriptomicCov[j].chr == chromosomesArray[k].id && (start == null || (transcriptomicCov[j].pos > start &&
          transcriptomicCov[j].pos < end))) {

        if (start) {
          chromosomesArray[k].positions[Math.floor((transcriptomicCov[j].pos - start) / regionSize)]+=parseFloat(transcriptomicCov[j].value);
        } else {
          chromosomesArray[k].positions[Math.floor(transcriptomicCov[j].pos / regionSize)]+=parseFloat(transcriptomicCov[j].value);
        }
      }
    }
  }
  //console.log(chromosomesArray[0],chromosomesArray[1])
...
module.exports = {
  setTranscriptomicCovbins:setTranscriptomicCovbins,
  updateTranscriptomicCovbins:updateTranscriptomicCovbins,
  serializeTranscriptomicCovbins:serializeTranscriptomicCovbins,
  calculateGenomeLength:calculateGenomeLength,
  calculateTranscriptomicCovDensity:calculateTranscriptomicCovDensity
}
