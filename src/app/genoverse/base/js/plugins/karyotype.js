Genoverse.Plugins.karyotype = function () {
  ...
  function createKaryotype() {
    ...
    container  = $('<div class="gv-karyotype-container">').html(chromosome).insertBefore(this.wrapper);
    chrDisplayed=this.chr;





    var chrContainer = container[0].childNodes[0];
    chrContainer =  chrContainer.getElementsByClassName('gv-wrapper');


    setTimeout(function(){
      chrContainer=chrContainer[0]
      chrContainer.className+=' gv-caryotype-chromosome';
    }, 100)
  ...
  this.on({
    ...
    afterSetRange: function () {
      ...
      if (this.karyotype) {
        if(this.chr!=chrDisplayed){
          chrDisplayed=this.chr;
          recreateKaryotype(this);
        }else{
