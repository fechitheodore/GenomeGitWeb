var Genoverse = Base.extend({
  ...
  loadGenome: function () {
    if (typeof this.genome === 'string') {
      ...
      return $.ajax({
        url      : 'genoverse/js/genomes/' + genomeName + '.js',
        dataType : 'script',
        context  : this,
        success  : function () {
  ...
  loadPlugins: function (plugins) {
    ...
    function loadPlugin(plugin) {
      var css      = 'genoverse/css/'+ plugin + '.css';
      var js       = 'genoverse/js/plugins/' + plugin + '.js';
  ...
  reset: function (settings) {
    ...
    if(settings.files.annotation){

      this.tracksById["genes"].urlParams= {fileID: settings.files.annotation};
      this.tracksById["genes"].model.urlParams = {fileID: settings.files.annotation};
      this.tracksById["genes"].models["-1"].urlParams = {fileID: settings.files.annotation};
      this.tracksById["genes"].models["1"].urlParams= {fileID: settings.files.annotation};
      this.tracksById["genes"].models["100000"].urlParams= {fileID: settings.files.annotation};
      this.tracksById["genes"].models["2000000"].urlParams= {fileID: settings.files.annotation};
    ...
    this.genome = {};
    this.genome[settings.chr.id]= {size: settings.chr.size, bands:[]};
    this.chromosomeSize=settings.chr.size;
